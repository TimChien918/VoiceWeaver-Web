import { state, newId, initAuth, loginGoogle, loginAnon, logout, save, addHistory, listHistory, toggleFavorite, ensurePairCode, pushNgrokBridge, saveShortcut, listVoices, reauthorizeDrive, needsDriveReauth, isBenignAuthError, accountEmail, switchAccount, needsScopeUpgrade } from "./store.js?v=1.5.76";
import { LLM_PROVIDERS, IMAGE_PROVIDERS, TTS_PROVIDERS, TTS_VOICES, testEntry, hasCloudTts } from "./providers.js?v=1.5.76";
import { initShared, sharedAvailable, sharedUsage, campaignActive, campaignStart, campaignEnd, campaignPersonal, campaignBoth } from "./shared.js?v=1.5.76";
import { WEBGPU_MODELS, webgpuSupported, probeWebgpu, webgpuEnabled, setWebgpuEnabled, webgpuModel,
         setWebgpuModel, webgpuReady, webgpuLoading, loadWebgpu, onWebgpuProgress } from "./webgpu.js?v=1.5.76";
import { reconstruct, composeAac, hasAnyLlmKey, classifyCrisisIntent } from "./llm.js?v=1.5.76";
import { speak, speakNow, speakIn, listen, sttSupported, setSpeechToast } from "./speech.js?v=1.5.76";
import { AAC_CATS, CAT_EMOJI, cardsOfCat, allCards, searchCards, CURRENCIES } from "./aac.js?v=1.5.76";
import { feed as rankFeed, rankWithin, recordUse, activeItemCount } from "./aacrank.js?v=1.5.76";
import { setupKiosk, enterKiosk } from "./kiosk.js?v=1.5.76";
import { bindTap } from "./interaction.js?v=1.5.76";
import { orderCards } from "./predict.js?v=1.5.76";
import { CLINICAL_BANK, practiceItem } from "./clinical.js?v=1.5.76";
import { markFirstSpeak, recordCandidateChoice, recordUndo, recordInputSource } from "./behavior.js?v=1.5.76";
import { openCrisis, setupCrisis } from "./crisis.js?v=1.5.76";
import { classifyRisk, containsCrisisSignal } from "./safety.js?v=1.5.76";
import { preloadZhConv, toTraditionalSync } from "./zhconv.js?v=1.5.76";
import { setupStory, renderStory, setStoryToast } from "./story.js?v=1.5.76";
import { setupHeadControl, stopHeadControl } from "./headcontrol.js?v=1.5.76";
import { startAudioCapture, stopAndInterpret, cancelAudioCapture, isRecording, hasNativeAudio } from "./audiodirect.js?v=1.5.76";
import { generateImage, intentPrompt, detectLocation, recognizePhoto, telegramNotify } from "./extras.js?v=1.5.76";
import { setupRehab, renderRehabLogs, setRehabToast } from "./rehab.js?v=1.5.76";
import { setupReport, loadReport, setReportToast } from "./report.js?v=1.5.76";
import { detectLocalTts, localVoices, localSwitch, localCatalog, localPrepare, localComputeEnabled, localHas } from "./localtts.js?v=1.5.76";
import { applyI18n, t } from "./i18n.js?v=1.5.76";
import { setupDemo } from "./demo.js?v=1.5.76";
 ...
 * topbar 上的活動標記。
 *
 * 跟上面那一則的差別是**給誰看**：那一則只寫給正在借金鑰的人，所以已經有自己
 * 金鑰的人（多半是照護者）完全不知道活動存在，也就不會想到可以叫還沒設定的
 * 家人趁這幾天直接開始用。這一顆對每個帳號都出現，只要活動還在。
 *
 * 顯示條件只看 campaignActive()，不看 sharedAvailable()：後者在「今天次數用完」
 * 時是 false，但活動並沒有結束——那時標記消失會讓人以為活動被取消了。
 * 次數的事情留給點下去的說明講。
 */
...
function showApp(user){
  ...
  applyTheme(); applyI18n(state.settings.lang);
  // 設定索引的每一列是從卡片的 h3 抄過來的，**applyI18n 掃不到它**（它是動態產生的）。
  // 雲端設定把語言換掉時不重建，索引就會卡在登入前那個語言——整頁都翻好了，
  // 只有設定的目錄還是英文。
  buildSettingsIndex();
  renderCampaignChip(); fillSettings(); renderFavorites();
  ...
  initShared().then(()=>{ renderSharedNote(); renderCampaignChip(); }).catch(()=>{});
  ...
  $("#campaignChip").addEventListener("click", showCampaignInfo);
...
async function renderVoices(){
  ...
  if(!list.length){
    ...
    try{
      const drive = await import("./drive.js?v=1.5.76");
